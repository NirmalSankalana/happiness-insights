...
GEOJSON_FILE = "./data/final.geojson"
...
@st.cache_data(hash_funcs={pd.DataFrame: lambda df: tuple(df.values)})
...
def display_base_map(_geo_data, df, myscale):
    ...
    map = folium.Map(location=[x_map, y_map],
                     zoom_start=1, max_bounds=True, min_zoom=2, tiles=None, scrollWheelZoom=False)
